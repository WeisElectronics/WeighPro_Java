...
public class JDatePanelImpl extends JPanel implements JDatePanel {
	...
	public JDatePanelImpl(DateModel<?> model) {
		...
		internalModel = new InternalCalendarModel((model != null) ? model
	...
	private class InternalView extends JPanel {
		...
		private void updateMonthLabel() {
			...
			monthLabel.setText(df.getMonths()[internalModel.getModel()
					.getMonth()]);
		}
		...
		private void initialise() {
			...
			this.setPreferredSize(new java.awt.Dimension(220, 200));
		...
		private JPanel getNorthPanel() {
			if (northPanel == null) {
				...
				northPanel.add(getPreviousButtonPanel(),
						java.awt.BorderLayout.WEST);
				northPanel
						.add(getNextButtonPanel(), java.awt.BorderLayout.EAST);
				northPanel.add(getNorthCenterPanel(),
		...
		private JPanel getNorthCenterPanel() {
			if (northCenterPanel == null) {
				...
				northCenterPanel.setBorder(javax.swing.BorderFactory
						.createEmptyBorder(0,0, 0, 0));
				northCenterPanel.setOpaque(false);
				northCenterPanel.setBackground(Color.black);
				northCenterPanel.add(getMonthLabel(),
						java.awt.BorderLayout.CENTER);
				//northCenterPanel.add(getYearSpinner(),
	...
	/*	private JSpinner getYearSpinner() {
			if (yearSpinner == null) {
				...
				yearSpinner.getEditor().setOpaque(false);
				((JSpinner.NumberEditor) yearSpinner.getEditor())
						.getTextField().setOpaque(false);
				yearSpinner.setModel(internalModel);
				yearSpinner.setForeground(Color.white);
				yearSpinner.setUI(new BasicSpinnerUI());
		...
		private JPanel getSouthPanel() {
			if (southPanel == null) {
				...
				southPanel.setBorder(javax.swing.BorderFactory
		...
		private JLabel getNoneLabel() {
			if (noneLabel == null) {
				...
				noneLabel.setFont(new Font(Font.DIALOG, Font.PLAIN, 14));
		...
		private JLabel getTodayLabel() {
			if (todayLabel == null) {
				...
				todayLabel.setFont(new Font(Font.DIALOG, Font.PLAIN, 14));
				DateFormat df1 = JDatePickerUtil.getMediumDateFormat();
				todayLabel.setText(i18nStrings.getProperty("messages.today")
						+ ": " + df1.format(new Date()));
			}
			return todayLabel;
		}
		...
		private JPanel getCenterPanel() {
			if (centerPanel == null) {
				...
				centerPanel.add(getDayTableHeader(),
		...
		private JTableHeader getDayTableHeader() {
			if (dayTableHeader == null) {
				...
				dayTable.getTableHeader().setOpaque(false);
		...
		private JButton getNextMonthButton() {
			if (nextMonthButton == null) {
				...
				nextMonthButton.setFont(new Font(Font.DIALOG, Font.PLAIN, 15));
				nextMonthButton
						.setPreferredSize(new java.awt.Dimension(20, 15));
				nextMonthButton.setBorder(new EmptyBorder(0, 0, 0, 0));
				nextMonthButton.setBackground(new Color(255, 255, 255, 0));
				nextMonthButton.setOpaque(false);
				nextMonthButton
						.setPreferredSize(new java.awt.Dimension(20, 15));
				nextMonthButton.setFocusable(false);
				nextMonthButton.addActionListener(internalController);
				nextMonthButton.setToolTipText(i18nStrings
						.getProperty("messages.nextMonth"));
			}
		...
		private JButton getNextYearButton() {
			if (nextYearButton == null) {
				nextYearButton = new javax.swing.JButton(new JNextIcon(8, 7,
						true));
				...
				nextYearButton.setToolTipText(i18nStrings
						.getProperty("messages.nextYear"));
			}
		...
		private JButton getPreviousMonthButton() {
			if (previousMonthButton == null) {
				...
				previousMonthButton.setFont(new Font(Font.DIALOG, Font.PLAIN,
						15));
				previousMonthButton.setPreferredSize(new java.awt.Dimension(20,
						15));
				previousMonthButton.setBorder(new EmptyBorder(0, 0, 0, 0));
				previousMonthButton.setBackground(new Color(255, 255, 255, 0));
				...
				previousMonthButton.setToolTipText(i18nStrings
						.getProperty("messages.previousMonth"));
			}
		...
		private JButton getPreviousYearButton() {
			if (previousYearButton == null) {
				previousYearButton = new javax.swing.JButton(new JPreviousIcon(
						8, 7, true));
				previousYearButton.setText("");
				previousYearButton.setPreferredSize(new java.awt.Dimension(20,
						15));
				previousYearButton
						.setBorder(javax.swing.BorderFactory
								.createBevelBorder(javax.swing.border.BevelBorder.RAISED));
				...
				previousYearButton.setToolTipText(i18nStrings
						.getProperty("messages.previousYear"));
			}
	...
	private class InternalTableCellRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable arg0,
				Object arg1, boolean isSelected, boolean hasFocus, int row,
				int col) {
			JLabel label = (JLabel) super.getTableCellRendererComponent(arg0,
					arg1, isSelected, hasFocus, row, col);
			...
			selectedCal.set(internalModel.getModel().getYear(), internalModel
					.getModel().getMonth(), internalModel.getModel().getDay());

			int cellDayValue = (Integer) arg1;
			int lastDayOfMonth = selectedCal
			...
			if (cellDayValue < 1 || cellDayValue > lastDayOfMonth) {
				...
				// Before start of month
					...
					lastMonth.set(selectedCal.get(Calendar.YEAR),
							selectedCal.get(Calendar.MONTH) - 1, 1);
			...
			// This month
				...
				if (todayCal.get(Calendar.DATE) == cellDayValue
						&& todayCal.get(Calendar.MONTH) == internalModel
								.getModel().getMonth()
						&& todayCal.get(Calendar.YEAR) == internalModel
								.getModel().getYear()) {
					label.setForeground(new Color(128, 0, 0));
					// Selected
					if (internalModel.getModel().isSelected()
							&& selectedCal.get(Calendar.DATE) == cellDayValue) {
	...
	private class InternalController implements ActionListener, MouseListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			if (arg0.getSource() == internalView.getNextMonthButton()) {
			...
			} else if (arg0.getSource() == internalView
				...
				for (int month = 0; month < internalView
						.getMonthPopupMenuItems().length; month++) {
					if (arg0.getSource() == internalView
							.getMonthPopupMenuItems()[month]) {
		...
		public void mousePressed(MouseEvent arg0) {
			if (arg0.getSource() == internalView.getMonthLabel()) {
				internalView.getMonthPopupMenu().setLightWeightPopupEnabled(
						false);
				internalView.getMonthPopupMenu().show(
						(Component) arg0.getSource(), arg0.getX(), arg0.getY());
			} else if (arg0.getSource() == internalView.getTodayLabel()) {
				Calendar today = Calendar.getInstance();
				internalModel.getModel().setDate(today.get(Calendar.YEAR),
