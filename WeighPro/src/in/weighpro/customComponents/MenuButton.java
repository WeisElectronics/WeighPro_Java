/* This class provides the pluggable menu button component
...
public class MenuButton extends AbstractButton {
	...
	public MenuButton(String buttonText, ActionListener commonActionListener) {
		...
		final ActionEvent actionEvent = new ActionEvent(this,
				ActionEvent.ACTION_PERFORMED, buttonText);//Action listener when this button is clicked
		...
		addActionListener(commonActionListener);
	...
	public void paintComponent(Graphics g) {
		...
		g2.fillRect(0, 0, getWidth(), getHeight());//Giving the button a rectangular shape
		g2.setFont(MenuFontGenerator.menuPanelFont);//We are using copper plate Gothic which is initialized in the custom fonts package
		g2.setColor(new Color(50, 50, 50));//Grey color for the button when it is not clicked
		FontMetrics size = g2.getFontMetrics();//For calculating the actual width of the text for the center text alignment
		...
		//if the mouse is hovered when the button is not in the clicked state then change the background
		...
		if (mouseHovered) {
			...
			g2.drawRect(1, 1, getWidth() - borderStrokeSize / 2 - 1,
					getHeight() - borderStrokeSize / 2 - 1);//drawing a border rectangle along the edges of the button
		}

	}

	//This method sets the clicked status of the button used to set the default selected option from the menu panel
